        ...
        if uploaded_file is not None:
            ...
            if st.button("Process File"):
                with st.spinner('Processing file...'):
                    results = process_file(os.path.join(database_path, uploaded_file.name))
            ...
            if not st.session_state['processed']:
                if youtube_link.find("list")==-1:
                    with st.spinner('Donwloading from Youtube'):
                        file_path = youtube_downloader(youtube_link, database_path)
                        st.session_state['file_path'] = file_path
                        st.session_state['processed'] = True
            if youtube_link.find("list")!=-1:
                st.warning("This page doesn't work with playlist, please give simple video url.")
